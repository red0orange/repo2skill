#!/usr/bin/env python3
"""
Large-scale parallel analysis with Alibaba Bailian API.
...
import config_bailian as config
...
class AsyncBailianClient:
    """Async Alibaba Bailian API client with rate limiting and retry."""
    ...
    async def call_api(self, messages: list) -> Optional[str]:
        """Call Bailian API with async and retry logic."""
        headers = {
...
{{
  ...
  "skillability_rationale": "<brief explanation>"
    ...
    def _get_github_prompt(self, repo: Dict[str, Any]) -> str:
        ...
        readme = repo.get('readme_content', '')[:3000]
...
async def process_batch(client: AsyncBailianClient, items: List[tuple],
    ...
    results = []
    for result, metadata in zip(task_results, metadatas):
        if result:
...
async def main():
    ...
    top_candidates = get_top_candidates(extracted_data, k=100)

...
AI agent ecosystems increasingly package software capabilities as reusable "skills" that agents can discover and invoke. Yet deciding which software projects are worth converting into skills is still largely ad hoc. We report an exploratory empirical study of 29,896 artifacts: 2,200 skills from the Clawhub marketplace and 27,696 GitHub repositories sampled from a larger filtered corpus. We operationalize **skillability** as a six-dimensional construct covering task clarity, interface clarity, composability, automation value, deployment friction, and operational risk. We annotate artifacts with an LLM-based coding pipeline over repository metadata and README excerpts, and validate a 200-item subsample with human assessment (87% agreement within +/-0.5 points).

Our results provide three main observations. First, marketplace skills receive higher skillability scores than sampled GitHub repositories (3.75 vs. 2.88; Delta = 0.87, Welch's t-test p < 0.001, Cohen's d = 0.74), indicating that artifacts already packaged for agents occupy a distinct and empirically recognizable region of the design space. Second, 35.8% of all analyzed artifacts and 32.6% of sampled GitHub repositories satisfy our high-skillability threshold (score >= 4.0), with Data Retrieval & Search, Multimedia Content, and System Infrastructure showing the highest concentration of promising candidates. Third, raw skillability is effectively independent of repository popularity (Spearman r_s = 0.003), suggesting that large pools of viable candidates remain undiscovered by popularity-driven curation alone. We further derive an opportunity-ranking heuristic that surfaces 9,033 GitHub repositories as promising skill-conversion candidates.

We position the paper as a scalable empirical foundation for future work on repository-to-skill pipelines rather than as a fully validated measurement paper. The study contributes a reusable analytic rubric, a large-scale characterization of agent-facing software, and concrete evidence that open-source ecosystems contain substantial room for systematic, potentially batch-oriented skillification.
...
![Figure 1. Distribution of skillability scores for Clawhub skills and GitHub repositories.](../output_large/figures/fig1_skillability_distribution.png)
...
![Figure 2. Mean skillability dimensions for Clawhub and GitHub with 95% confidence intervals.](../output_large/figures/fig_new1_dimension_comparison_ci.png)
...
![Figure 3. Mean skillability by capability category in the GitHub sample, ranked with 95% confidence intervals and sample sizes.](../output_large/figures/fig_new2_category_ranking.png)
...
![Figure 4. Proportion of high-skillability repositories (SS >= 4.0) by capability category in the GitHub sample.](../output_large/figures/fig_new3_high_skillability_by_category.png)
...
![Figure 5. Top-ranked GitHub repositories by opportunity score, colored by capability category and labeled with stars and skillability scores.](../output_large/figures/fig_new4_top_opportunities.png)
...
![Figure 6. Skillability scores versus GitHub stars (log scale) for sampled repositories, highlighting the near-zero rank correlation.](../output_large/figures/fig8_stars_vs_skillability.png)
...
The broader significance of the work is its extensibility. Beyond the descriptive findings, the paper establishes that repository-scale discovery of promising skill candidates is feasible, that the candidate pool is large enough to matter, and that the opportunity space is structured enough to support targeted conversion strategies. We therefore see this work as a strong front-end stage for future repository-to-skill pipelines: identifying where large-scale or semi-automated skillification is plausible, what kinds of repositories should be prioritized first, and what technical bottlenecks future systems should address.

We deliberately do not present skillability as a finished measurement instrument. The contribution is instead a structured empirical starting point: a rubric, a large-scale annotated corpus, ecosystem findings with direct operational value, and a prioritization heuristic that can support both curation and the next generation of large-scale skill-construction research.

...
### 3.7 Predictive Validation

To validate whether skillability scores predict real-world skill adoption, we conducted a binary classification study comparing Clawhub marketplace skills (n=2,200) against GitHub repositories (n=27,696).

**Data Integration**: We matched Clawhub skills to their source GitHub repositories via URL extraction and slug matching (100% match rate). For GitHub repositories, we extracted metadata (stars, language, license, archived status) and computed skillability scores using the same LLM-based annotation pipeline.

**Feature Sets**: We compared two feature sets: (1) Baseline: repository metadata only (has_license, archived), and (2) Full: baseline + all six skillability dimensions + capability/granularity categories (38 features total).

**Models**: We trained Logistic Regression, Random Forest, and XGBoost classifiers with class balancing (scale_pos_weight for imbalanced data: 12.6:1 ratio). Train/test split: 80/20 stratified.

**Evaluation**: ROC-AUC and PR-AUC on held-out test set. Statistical significance via bootstrap confidence intervals (1,000 samples) and McNemar's test. Dimension importance via SHAP values and ablation studies (single-dimension models).
...
### 5.5 RQ5: Predictive Validation

To validate whether skillability scores predict real-world skill adoption, we trained binary classifiers to distinguish Clawhub marketplace skills from GitHub repositories.

**Baseline Performance**: Using only repository metadata (has_license, archived), XGBoost achieved AUC=0.8801 (95% CI [0.8701, 0.8901]), indicating that basic quality signals partially predict skill adoption.

**Full Model Performance**: Adding skillability dimensions improved performance to AUC=0.9863 (95% CI [0.9831, 0.9894]), representing a significant lift of +0.1061 (p<0.001, bootstrap CI [0.1006, 0.1116]). PR-AUC improved from 0.2489 to 0.8360. McNemar's test showed +1,074 net correct predictions (p<0.001).

**Dimension Importance**: SHAP analysis revealed automation_value (0.953), composability (0.573), and task_clarity (0.400) as top predictors. Ablation studies confirmed these dimensions achieve strong standalone performance: automation_value (AUC=0.776), composability (AUC=0.722), task_clarity (AUC=0.685).

**Robustness**: Model performance remained stable across capability categories (AUC range: 0.035) and cross-validation splits (AUC=0.986±0.001), confirming generalization.

Figure 9 shows ROC and Precision-Recall curves comparing baseline and full models, demonstrating the substantial performance improvement from adding skillability features.

![Figure 9: ROC and PR Curves for Predictive Validation. Left: ROC curves showing baseline (AUC=0.8801) vs. full model (AUC=0.9863). Right: Precision-Recall curves showing baseline (AUC=0.2489) vs. full model (AUC=0.8360). The full model incorporating skillability dimensions significantly outperforms the baseline using only repository metadata.](../output/validation/figures/roc_pr_curves.png)

Figure 10 presents feature importance rankings and SHAP values, highlighting automation_value, composability, and task_clarity as the most predictive dimensions.

![Figure 10: Feature Importance and SHAP Analysis. Top: XGBoost feature importance for top 15 features. Bottom: SHAP importance values showing mean absolute SHAP contributions. Automation_value, composability, and task_clarity emerge as strongest predictors of marketplace adoption.](../output/validation/figures/feature_importance.png)

Figure 11 shows ablation study results, confirming that individual skillability dimensions achieve strong standalone predictive performance.

![Figure 11: Ablation Study Results. Bar chart showing AUC scores for models trained on individual skillability dimensions. Automation_value (0.776), composability (0.722), and task_clarity (0.685) demonstrate strong standalone performance, validating their predictive value.](../output/validation/figures/ablation_study.png)
...
Our exploratory findings suggest: (1) existing agent skills exhibit different characteristics than general GitHub repositories across all six dimensions, though this comparison is confounded by documentation and selection differences; (2) approximately 35.8% of analyzed projects demonstrate high skillability (score ≥4.0); (3) task clarity and automation value are the strongest contributors to the composite skillability score; (4) skillability is essentially uncorrelated with repository popularity (r_s=0.003), suggesting many promising projects remain undiscovered; and (5) skillability scores successfully predict marketplace skill adoption (AUC=0.986), with automation_value, composability, and task_clarity as strongest predictors.
